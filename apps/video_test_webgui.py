...
from crossmedia_pid.dataset import default_dataset_name, export_track_dataset
...
def init_crossmedia_pid():
    """初始化CrossMedia-PID系统"""
    try:
        pid = CrossMediaPID(load_config())
    except Exception as exc:
        st.error(f"CrossMedia-PID 初始化失败: {exc}")
        return None
    
    return {
        'extractor': pid.extractor,
        'feature_extractor': pid.feature_extractor,
        'vectorizer': pid.vectorizer,
        'store': pid.store,
        'matcher': pid.matcher
    }
...
def main():
    ...
    if 'dataset_export' not in st.session_state:
        st.session_state.dataset_export = None
    if 'dataset_name' not in st.session_state:
        st.session_state.dataset_name = default_dataset_name()
    ...
    if st.session_state.tracks:
        ...
        # 步骤3: 创建测试集
        st.markdown('<div class="section-header">🗂️ 步骤3: 创建测试集</div>', unsafe_allow_html=True)
        ...
        with st.container():
            ...
            with col_a:
                dataset_name = st.text_input(
                    ...
                    help="导出目录名会自动清理为安全文件名"
                )
            with col_b:
                min_track_frames = st.number_input(
                    "最少轨迹帧数",
                    min_value=1,
                    max_value=500,
                    value=2,
            ...
            with col_c:
                samples_per_track = st.slider("每个身份抽样数量", 1, 12, 4)
            with col_d:
                crop_padding = st.slider("裁剪扩边比例", 0.0, 0.4, 0.1, step=0.05)
            with col_e:
                negative_pairs = st.slider("负样本对/身份组合", 0, 10, 2)
            ...
            default_track_ids = candidate_track_ids[: min(3, len(candidate_track_ids))]
            selected_track_ids = st.multiselect(
                "选择要导出的身份轨迹",
                options=candidate_track_ids,
                default=default_track_ids,
                format_func=lambda track_id: (
                    f"人物 #{track_id} - {len(tracks[track_id].frame_history)} 帧, "
                    f"质量 {tracks[track_id].best_quality:.3f}"
                )
            )

            if st.button("📦 生成测试集", type="primary", use_container_width=True):
                try:
                    with st.spinner("正在裁剪图片并生成 manifest/pairs..."):
                        export_result = export_track_dataset(
                            ...
                            negative_pairs_per_identity=negative_pairs,
                        )
                    st.session_state.dataset_name = dataset_name
                    st.session_state.dataset_export = export_result
                    st.success(f"✅ 测试集已生成: {export_result.dataset_dir}")
                except Exception as exc:
                    st.error(f"测试集生成失败: {exc}")

            if st.session_state.dataset_export:
                export_result = st.session_state.dataset_export
                metric_cols = st.columns(4)
                metric_cols[0].metric("身份数", export_result.identity_count)
                metric_cols[1].metric("图片数", export_result.image_count)
                metric_cols[2].metric("正样本对", export_result.positive_pair_count)
                metric_cols[3].metric("负样本对", export_result.negative_pair_count)

                st.caption(f"输出目录: {export_result.dataset_dir}")
                if export_result.zip_path.exists():
                    st.download_button(
                        "下载测试集 ZIP",
                        data=export_result.zip_path.read_bytes(),
                        file_name=export_result.zip_path.name,
        ...
        if st.session_state.selected_track_id is not None:
            ...
            st.markdown('<div class="section-header">🧪 步骤4: CrossMedia-PID匹配测试</div>', unsafe_allow_html=True)
